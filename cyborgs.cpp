...
void clearScreen();
//
int aroundFour(const Arena& a, int r, int c);
...
{
      // TODO: TRIVIAL:  Return what column the cyborg is at.
...
{
      // TODO: TRIVIAL:  Return what channel the cyborg is listening on.
...
{
      // TODO: TRIVIAL:  Return whether the cyborg is dead.
    if (m_health == 0) {
        return true; }
    else {
        return false; }
...
string Player::move(int dir)
{
    ...
    if (dir == NORTH) {
        if (m_arena->numberOfCyborgsAt(m_row-1, m_col) >= 1) {
            m_row--;
            m_dead = true;
            return "Player walked into a cyborg and died."; }
//        else if (m_arena->hasWallAt(m_row-1, m_col)) {
//            return "Player couldn't move; player stands."; }
        else if (player->m_row-1 > 0){
            if (attemptMove(*m_arena, NORTH, m_row, m_col) == true) {
...
//        else if (m_arena->hasWallAt(m_row+1, m_col)) {
//            return "Player couldn't move; player stands."; }
        else if (player->m_row+1 <= m_arena->rows()) {
            if (attemptMove(*m_arena, SOUTH, m_row, m_col) == true) {
            return "Player moved south."; }
        }}
    
    if (dir == WEST) {
        if (m_arena->numberOfCyborgsAt(m_row, m_col-1) >=1) {
            m_col--;
            m_dead = true;
            return "Player walked into a cyborg and died."; }
//        else if (m_arena->hasWallAt(m_row, m_col-1)) {
//            return "Player couldn't move; player stands."; }
        else if (player->m_col-1 > 0) {
            if (attemptMove(*m_arena, WEST, m_row, m_col) == true) {
...
//        else if (m_arena->hasWallAt(m_row, m_col+1)) {
//            return "Player couldn't move; player stands."; }
        else if (player->m_col+1 <= m_arena->cols()){
            if (attemptMove(*m_arena, EAST, m_row, m_col) == true) {
...
{
      // TODO:  Return the number of cyborgs at row r, column c.
    ...
    for (int k = 0; k < m_nCyborgs; k++) {
        if (m_cyborgs[k] == nullptr) {
            continue;}
        else if (m_cyborgs[k]->row() == r && m_cyborgs[k]->col() == c) {
            AmtofCyborgs++;
}
    }
    return AmtofCyborgs;
    ...
    if (willRespond == true) {
        for (int i = 0; i < m_nCyborgs; i++) {
            if (m_cyborgs[i] == nullptr) {
                continue; }
            if (channel == m_cyborgs[i]->channel()) {
                ...
                if (m_cyborgs[i]-> isDead()){
                    delete m_cyborgs[i];
                    m_cyborgs[i] = nullptr;
                    for (int k=i; k < m_nCyborgs-1; k++){
                        m_cyborgs[k] = m_cyborgs[k+1]; }
                    m_nCyborgs--;
                        }
            } }}

    if (willRespond == false) {
...
string Game::takePlayerTurn()
{
    for (;;)
    {
        ...
        {
            if (
            (*m_arena, player->row(), player->col(), dir))
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
    ...
    if (a.numberOfCyborgsAt(r-1, c) > 0 && a.numberOfCyborgsAt(r+1, c) > 0 && a.numberOfCyborgsAt(r, c-1) > 0 && a.numberOfCyborgsAt(r, c+1) > 0) {
        return false; }
    
    if (a.numberOfCyborgsAt(r-1, c) > 0 && a.numberOfCyborgsAt(r+1, c) > 0 && a.numberOfCyborgsAt(r, c-1) > 0 && a.hasWallAt(r, c+1)) {
        return false; }
    ...
    if (r-1 >= 1) {
        if (a.numberOfCyborgsAt(r-1, c) == 0 && !a.hasWallAt(r - 1, c)) {
            danger = aroundFour(a, r-1, c);
                if (danger < maxriskiness) {
                    bestDir = NORTH;
                    maxriskiness = danger;
            }
    }
    ...
    if (c+1 <= a.cols()) {
        if (a.numberOfCyborgsAt(r, c+1) == 0 && !a.hasWallAt(r, c+1)) {
            danger = aroundFour(a, r, c+1);
            if (danger < maxriskiness) {
                bestDir = EAST;
                maxriskiness = danger; }
            }
...
//    int number = 1;
//    if (c == a.cols() || r == a.rows() || c == 1 || r == 1 ) {
//        number = 0;}
    
    int risk = 0;
    if (!(r == 1)) {
    if (a.numberOfCyborgsAt(r-1 , c) > 0) { //NORTH
         risk++; } }
    
    if (!(r == a.rows())) {
    if (a.numberOfCyborgsAt(r+1, c) > 0) { //SOUTH
        risk++; } }
        
    if (!(c == 1)) {
    if (a.numberOfCyborgsAt(r, c-1) > 0) { //WEST
        risk++; } }
    
    if (!(c == a.cols())) {
    if (a.numberOfCyborgsAt(r, c+1) > 0) { //EAST
        risk++; } }
    return risk;
}
